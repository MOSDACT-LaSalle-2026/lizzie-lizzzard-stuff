...
// --- timer ---
int huntStartTime = 0;
int TIME_LIMIT    = 20000;   // 20 seconds in milliseconds -- change this to make it harder/easier
...
void huntforappointments() {
  ...
  if (elapsed >= TIME_LIMIT) {
    gameOver = true;
    phase    = 5;   // time's up --> sorry screen
  }
  ...
  // --- draw countdown bar at the top ---
  float progress = 1.0 - constrain((float)elapsed / TIME_LIMIT, 0, 1);
  noStroke();
  fill(40);
  rect(0, 0, width, 8);
  fill(lerpColor(color(216, 90, 48), color(83, 200, 100), progress));
  rect(0, 0, width * progress, 8);
