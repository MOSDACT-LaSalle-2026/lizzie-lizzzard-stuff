...
// --- timer ---
int huntStartTime = 0;
int TIME_LIMIT    = 15000;   // 15 seconds -- lower = harder
...
void huntforappointments() {
  ...
  // update and draw grey distractions
  ...
  // --- WANDER ---
  // in last 5 seconds: slower drift so the ball is actually catchable
  ...
  // --- speed cap (much lower in last 5 seconds) ---
...
void revisecatches() {
  ...
  if (dist < BALL_RADIUS) {
    ...
    phase      = 3;
    gameOver   = true;
    winTime    = millis();   // start the 5-second win screen clock
  }
}
